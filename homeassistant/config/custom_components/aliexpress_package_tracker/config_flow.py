    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            return self.async_create_entry(
                ...
                data=user_data,
                options=user_data,  # options can be initialized with same data
            )
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_AUTO_DELETE_DAYS, default=7): vol.All(
...
class AliexpressOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry):
        ...
        # Use a private attribute to avoid deprecated assignment
    ...
    async def async_step_init(self, user_input=None):
        ...
        current_language = self._config_entry.data.get(CONF_LANGUAGE, self.hass.config.language)
        current_auto_delete = self._config_entry.data.get(CONF_AUTO_DELETE, False)
        current_auto_delete_days = self._config_entry.data.get(CONF_AUTO_DELETE_DAYS, 7)
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_AUTO_DELETE_DAYS, default=current_auto_delete_days): vol.All(
                        vol.Coerce(int), vol.Range(min=1, max=365)
                    ),
                }
            ),
